...
class TransferVoucherViewSet(viewsets.ModelViewSet):
    ...
    def post_transfer(self, request, pk=None):
        ...
        try:
            res = services.post_transfer(actor=request.user, voucher_id=voucher.id)
    ...
    def receive_transfer(self, request, pk=None):
        voucher = self.get_object()
        try:
            res = services.receive_transfer(actor=request.user, voucher_id=voucher.id)
            return Response(res, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def cancel_transfer(self, request, pk=None):
        voucher = self.get_object()
        try:
            res = services.cancel_transfer(actor=request.user, voucher_id=voucher.id)
            return Response(res, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
